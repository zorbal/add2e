 ...
 */
export class Add2eActorSheetMonster extends Add2eActorSheet {
  constructor(...args) {
    super(...args);
  }
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["add2e", "sheet", "monster", "actor"],
      template: "systems/add2e/templates/actors/monster-sheet.html",
      width: 450,
      height: 560,
      resizable: true,
      tabs: [
   ...
   */
  async generateSave() {
    let choices = CONFIG.ADD2E.monster_saves;

    let templateData = { choices: choices },
      dlg = await renderTemplate(
        "/systems/add2e/templates/actors/dialogs/monster-saves.html",
        templateData
      );
    //Create Dialog window
    new Dialog({
      title: game.i18n.localize("ADD2E.dialog.generateSaves"),
      content: dlg,
      buttons: {
        ok: {
  ...
  getData() {
    ...
    // Settings
    data.config.morale = game.settings.get("add2e", "morale");
  ...
  async _chooseItemType(choices = ["weapon", "armor", "shield", "gear"]) {
    let templateData = { types: choices },
      dlg = await renderTemplate(
        "systems/add2e/templates/items/entity-create.html",
        templateData
      );
    //Create Dialog window
    return new Promise((resolve) => {
      new Dialog({
        title: game.i18n.localize("ADD2E.dialog.createItem"),
        content: dlg,
        buttons: {
          ok: {
            label: game.i18n.localize("ADD2E.Ok"),

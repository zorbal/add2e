import { Add2eActor } from "./entity.js";
import { Add2eActorSheet } from "./actor-sheet.js";
import { Add2eCharacterModifiers } from "../dialog/character-modifiers.js";
import { Add2eCharacterCreator } from "../dialog/character-creation.js";

/**
 * Extend the basic ActorSheet with some very simple modifications
 */
export class Add2eActorSheetCharacter extends Add2eActorSheet {
  constructor(...args) {
    super(...args);
  }
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["add2e", "sheet", "actor", "character"],
      template: "systems/add2e/templates/actors/character-sheet.html",
      width: 450,
      height: 530,
      resizable: true,
      tabs: [
  ...
  generateScores() {
    new Add2eCharacterCreator(this.actor, {
  ...
  getData() {
    ...
    data.config.ascendingAC = game.settings.get("add2e", "ascendingAC");
    data.config.initiative = game.settings.get("add2e", "initiative") != "group";
    data.config.encumbrance = game.settings.get("add2e", "encumbranceOption");
    data.config.dexterityInitiative = game.settings.get("add2e", "dexterityInitiative");
  ...
  async _chooseLang() {
    let choices = CONFIG.ADD2E.languages;

    let templateData = { choices: choices },
      dlg = await renderTemplate(
        "/systems/add2e/templates/actors/dialogs/lang-create.html",
        templateData
      );
    ...
    return new Promise((resolve) => {
      new Dialog({
        ...
        buttons: {
          ok: {
            label: game.i18n.localize("ADD2E.Ok"),
  ...
  _pushLang(table) {
    ...
    this._chooseLang().then((dialogInput) => {
      const name = CONFIG.ADD2E.languages[dialogInput.choice];
      if (update.value) {
  ...
  _onShowModifiers(event) {
    event.preventDefault();
    new Add2eCharacterModifiers(this.actor, {
      top: this.position.top + 40,
      left: this.position.left + (this.position.width - 400) / 2,
    }).render(true);
  }

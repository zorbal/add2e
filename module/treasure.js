export const augmentTable = (table, html, data) => {
  ...
  const flag = table.object.getFlag("add2e", "treasure");
  const treasure = flag
  ...
  html.find(".toggle-treasure").click((ev) => {
    let isTreasure = table.object.getFlag("add2e", "treasure");
    table.object.setFlag("add2e", "treasure", !isTreasure);
  });
  ...
  if (flag) {
    ...
    // Replace Roll button
    const roll = `<button class="roll-treasure" type="button"><i class="fas fa-gem"></i> ${game.i18n.localize('ADD2E.table.treasure.roll')}</button>`;
...
function drawTreasure(table, data) {
  ...
  data.treasure = {};
  if (table.getFlag('add2e', 'treasure')) {
...
async function rollTreasure(table, options = {}) {
  ...
  let html = await renderTemplate(
    "systems/add2e/templates/chat/roll-treasure.html",
    templateData
  );

  let chatData = {
    content: html,
    // sound: "/systems/add2e/assets/coins.mp3"
  }

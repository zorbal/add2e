...
export class Add2eItem extends Item {
  ...
  prepareData() {
    ...
    switch (this.data.type) {
      case "spell":
        img = "/systems/add2e/assets/default/spell.png";
        break;
      case "ability":
        img = "/systems/add2e/assets/default/ability.png";
        break;
      case "armor":
        img = "/systems/add2e/assets/default/armor.png";
        break;
      case "weapon":
        img = "/systems/add2e/assets/default/weapon.png";
        break;
      case "item":
        img = "/systems/add2e/assets/default/item.png";
        break;
    }
  ...
  async rollFormula(options = {}) {
    ...
    return Add2eDice.Roll({
      ...
      flavor: game.i18n.format("ADD2E.roll.formula", { label: label }),
      title: game.i18n.format("ADD2E.roll.formula", { label: label }),
    });
  }
  ...
  getTags() {
    ...
    switch (this.data.type) {
      case "weapon":
        let wTags = formatTag(data.damage.medium + "/" + data.damage.large, "fa-tint");
        wTags += formatTag(data.speed, "fa-clock");
        data.tags.forEach((t) => {
          wTags += formatTag(t.value);
        });
        wTags += formatTag(CONFIG.ADD2E.saves_long[data.save], "fa-skull");
        if (data.missile) {
          wTags += formatTag(
            data.range.short + "/" + data.range.medium + "/" + data.range.long,
            "fa-bullseye"
          );
          wTags += formatTag("RoF " + data.rof);
        }
        return wTags;
      //case "armor":
        //return `${formatTag(CONFIG.ADD2E.armor[data.type], "fa-tshirt")}`;
      case "item":
        return "";
      case "spell":
        let sTags = `${formatTag(
          data.range, "fa-ruler"
        )}${formatTag(data.duration, "fa-hourglass")}${formatTag(data.areaofeffect, "fa-cube")}${formatTag(data.castingtime, "fa-clock")}${formatTag(data.roll)}`;
        if (data.save) {
          sTags += formatTag(CONFIG.ADD2E.saves_long[data.save], "fa-skull");
        }
        data.tags.forEach((t) => {
          sTags += formatTag(t.value);
        });
        return sTags;
      case "ability":
        let roll = "";
        roll += data.roll ? data.roll : "";
        roll += data.rollTarget ? CONFIG.ADD2E.roll_type[data.rollType] : "";
  ...
  pushTag(values) {
    ...
    if (update) {
      values.forEach((val) => {
        ...
        switch (val) {
          case CONFIG.ADD2E.tags.melee:
            newData.melee = true;
            break;
          case CONFIG.ADD2E.tags.slow:
            newData.slow = true;
            break;
          case CONFIG.ADD2E.tags.missile:
  ...
  async show() {
    ...
    const template = `systems/add2e/templates/chat/item-card.html`;

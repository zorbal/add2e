...
export class Add2eCharacterCreator extends FormApplication {
  static get defaultOptions() {
    const options = super.defaultOptions;
    options.classes = ["add2e", "dialog", "creator"],
      options.id = 'character-creator';
    options.template =
      'systems/add2e/templates/actors/dialogs/character-creation.html';
    options.width = 235;
    return options;
  }
   ...
   */
  get title() {
    return `${this.object.name}: ${game.i18n.localize('ADD2E.dialog.generator')}`;
  }
  ...
  rollScore(score, options = {}) {
    ...
    const label = score != "gold" ? game.i18n.localize(`ADD2E.scores.${score}.long`) : "Gold";
    const rollParts = ["3d6"];
    const data = {
      roll: {
        type: "result"
    ...
    return Add2eDice.Roll({
      ...
      flavor: game.i18n.format('ADD2E.dialog.generateScore', { score: label, count: this.object.data.counters[score] }),
      title: game.i18n.format('ADD2E.dialog.generateScore', { score: label, count: this.object.data.counters[score] }),
    });
  }
  ...
  async close() {
    ...
    const templateData = {
      config: CONFIG.ADD2E,
      scores: scores,
      title: game.i18n.localize("ADD2E.dialog.generator"),
      stats: this.object.data.stats,
      gold: gold
    }
    const content = await renderTemplate("/systems/add2e/templates/chat/roll-creation.html", templateData)
    ChatMessage.create({
      content: content,
      speaker,
    });
  ...
  async _onSubmit(event, { updateData = null, preventClose = false, preventRender = false } = {}) {
    ...
    const itemData = {
      name: "GP",
      type: "item",
      img: "/systems/add2e/assets/gold.png",
      data: {
        treasure: true,
        cost: 1,
        weight: 1,
